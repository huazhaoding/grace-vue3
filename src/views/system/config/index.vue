<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="配置组别" prop="configGroup">
        <el-select
    ...
    >
      ...
      <el-table-column
        ...
        :show-overflow-tooltip="true"
      />
      <el-table-column label="参数类型" align="center" prop="configValueType">
        <template #default="scope">
          <dict-tag
            :options="sys_config_value_type"
            :value="scope.row.configValueType"
          />
        </template>
      </el-table-column>
      <el-table-column
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="configRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="参数类型" prop="configValueType">
          <el-radio-group v-model="form.configValueType">
            <el-radio
              v-for="dict in sys_config_value_type"
              :key="dict.value"
...
const { sys_yes_no, sys_config_value_type } = proxy.useDict(
  "sys_yes_no",
...
const data = reactive({
  ...
  rules: {
    ...
    configGroup: [
      { required: true, message: "配置组不能为空", trigger: "blur" },
    ],
    configValueType: [
...
/** 初始化分组*/
function initGroup() {
  proxy.getConfigGroups().then((response) => {
    const groups = response.data.filter((item) => item != "");
    groups.forEach((g) => {
      sugGroups.value.push({ value: g });
    });
  });
}

function groupSearch(keyword, cb) {
  const results = keyword
    ? sugGroups.value.filter(createFilter(keyword))
    : sugGroups.value;
...
function createFilter(keyword) {
  return (item) => {
    return item.value.toLowerCase().indexOf(keyword.toLowerCase()) === 0;
  };
}
