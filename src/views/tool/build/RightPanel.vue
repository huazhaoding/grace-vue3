      ...
      <el-scrollbar class="right-scrollbar">
        ...
        <el-form v-show="currentTab === 'field' && showField" size="default" label-width="90px" label-position="top">
          ...
          <template v-if="activeDataProperty.tag === 'el-row'">
            <draggable :list="activeDataProperty.slots.default.dateOptions" :group="{ name: 'formItemChild', pull: false, put: false }"
              :sort="true" item-key="renderKey" class="draggable" @start="onDragStart" @end="onDragEnd">
              <template #item="{ element, index }">
                <el-card style="margin-bottom: 10px;">
                  <template #header>
                    <div class="card-header">
                      <span>栅格{{ index + 1 }}配置</span><el-button-group style="float: right">
                        <el-button type="primary" icon="Plus" @click="addCol(element, index)" title="复制栏" />
                        <el-button type="danger" icon="Remove" @click="removeCol(element, index)" title="删除栏" />
