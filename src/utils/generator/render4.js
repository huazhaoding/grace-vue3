...
function renderComponent(component, h, key) {
  h(
    resolveComponent(component.tag),
    {
      ...simplifyItem(component.attr),
      onClick: (event) => {
        event.stopPropagation(); // 阻止事件冒泡
        if (activeItem) {
          activeItem(item); // 调用从父组件传入的 activeItem 方法
  ...
  render() {
    ...
    if (childObjs) {
      Object.keys(childObjs).forEach((key) => {
        ...
        if (component[key]) {
          children.push(childFunc(h, component, key, this.activeItem));
        }
      });
    }
    //插槽内容
